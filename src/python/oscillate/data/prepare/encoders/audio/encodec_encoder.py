...
class EncodecEncoder(Encoder):
	...
	def __init__(self, bandwidth: int = 12.0, eos_token: int = 0):
	...
	def encode(self, inputs: typing.Any) -> np.ndarray:
		...
		eos_token = np.zeros((1, tokens.shape[1])) + self.__eos_token
		tokens = np.transpose(np.concatenate((tokens, eos_token), axis=0))
		return tokens

	def get_embedding_size(self) -> int:
		return int(self.__model.bandwidth * 4/3) + 1
